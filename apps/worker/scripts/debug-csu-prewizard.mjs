/**
 * Headed step-by-step CSU pre-wizard debug.
 *
 *   cd apps/worker
 *   pnpm capture:csu-session          # once
 *   node scripts/debug-csu-prewizard.mjs
 *
 * Env:
...
const FORM_URL = 'https://csu.17gz.org/apply/index.do';
const MEMBER_URL = 'https://csu.17gz.org/member/index.do';
const universityId = 'csu';
const outDir = resolve(process.cwd(), 'data/captures/csu-debug');
...
if (!storageState) {
  console.error('No CSU session. Run: pnpm --filter worker capture:csu-session');
  process.exit(1);
}

const metaPath = resolve(process.cwd(), 'browser-sessions/csu-meta.json');
const meta = existsSync(metaPath)
  ? JSON.parse(readFileSync(metaPath, 'utf-8'))
  : {};

console.log('=== CSU pre-wizard DEBUG ===');
...
if (/CSRF|Sign out|Account Sign In/i.test(state.body)) {
  console.error('SESSION DEAD — capture:csu-session again');
  await browser.close();
  process.exit(1);
}
...
// --- STEP: program type (SSP) ---
if (state.screen === 'program_type' || state.radios > 0 && state.radios < 5) {
...
// --- STEP: student type ---
console.log('\n>>> STEP student_type: Undergraduate label + Next');
state = await dump('04-before-student');

const undergrad = page
  .locator('label:has(input[name="projectTypeId"])')
  .filter({ hasText: /Undergraduate Student/i })
  .first();

console.log('  label count:', await undergrad.count());
if ((await undergrad.count()) > 0) {
  await undergrad.click({ force: true });
} else {
  // index 2 = Undergraduate on CSU
  await page.locator('input[name="projectTypeId"]').nth(2).click({ force: true });
}
...
if (state.screen === 'program_selection' || state.collegeId) {
  console.log('\n>>> STEP study plan: click first Apply (no filter guess)');
  const apply = page
    .locator('a')
    .filter({ hasText: /^(Apply|申请)$/i })
    .first();
