...
const META_FILE = resolve(process.cwd(), 'zzu-session-meta.json');
const SESSION_FILE = resolve(process.cwd(), 'zzu-session.json');
const DEFAULT_PROFILE_DIR = resolve(process.cwd(), 'zzu-browser-profile');

export function getZzuBrowserOptions() {
  const profileDir = process.env.ZZU_PROFILE_DIR || DEFAULT_PROFILE_DIR;
  const profileExists = existsSync(profileDir);

  return {
    headless: process.env.ZZU_HEADLESS === '1',
    channel: process.env.ZZU_BROWSER_CHANNEL || 'chrome',
    profileDir,
    profileExists,
    useProfile:
      process.env.ZZU_USE_PROFILE === '1' ||
      (profileExists && process.env.ZZU_USE_PROFILE !== '0'),
    cdpUrl: process.env.ZZU_USE_CDP === '1' ? process.env.ZZU_CDP_URL : undefined,
  };
}
...
export function loadZzuSession() {
  const b64 = process.env.ZZU_SESSION_STATE_B64;
  if (b64) {
...
export async function createZzuBrowserSession(
  storageState,
  { forCapture = false, preferStorageState = false } = {},
) {
  const options = getZzuBrowserOptions();
  const meta = loadSessionMeta();
  const state = storageState ?? loadZzuSession();

  if (forCapture) {
    mkdirSync(options.profileDir, { recursive: true });

    if (process.env.ZZU_USE_PROFILE !== '0') {
  ...
  if (options.useProfile && !preferStorageState) {
    ...
    try {
      ...
      console.warn(
        'Profile busy or locked — falling back to storageState. Close capture browser if open.',
      );
    }
  }
...
export async function closeZzuBrowserSession(session) {
  if (!session.closable || process.env.ZZU_KEEP_OPEN === '1') {
    return;
  }
...
export async function persistZzuSession(context) {
  const meta = loadSessionMeta();
  ...
  meta.mode = process.env.ZZU_USE_PROFILE === '1' ? 'profile' : 'storageState';
