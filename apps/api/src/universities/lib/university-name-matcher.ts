...
export function matchUniversityName(
  rawName: string,
  entries: UniversityMatchEntry[],
): {
  ...
  return {
    universityId:
      bestMatch && bestMatch.score >= AUTO_MATCH_THRESHOLD ? bestMatch.id : null,
    candidates: scored.slice(0, 5),
  };
}

function scoreUniversityEntry(
  normalizedRaw: string,
  entry: UniversityMatchEntry,
): number {
